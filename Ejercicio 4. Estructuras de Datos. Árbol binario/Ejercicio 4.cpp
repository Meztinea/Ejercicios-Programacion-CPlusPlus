...
int main() {
	...
	do {
		...
		std::cout << "4. Recorrido Inorden" << std::endl;
		std::cout << "5. Recorrido Preorden" << std::endl;
	...
	if (arbol == NULL) {
		...
		if (arbol->izq == NULL) {
			...
			if (femenino == "s" || femenino == "S") {
				...
				insertarNodo(arbol->izq, ascendenteFem);
		...
		if (arbol->der == NULL) {
			...
			if (masculino == "s" || masculino == "S") {
				...
				insertarNodo(arbol->der, ascendenteMas);
...
bool buscarNodo(Nodo *arbol, std::string nombre) {
	if (arbol == NULL) {
		return false;
	}
		...
		bool encontradoIzq = buscarNodo(arbol->izq, nombre);
        bool encontradoDer = buscarNodo(arbol->der, nombre);
...
void buscar() {
	std::string nombre;
	std::cout << "Digita el nombre a buscar: ";
	std::cin >> nombre;
	bool esEncontrado = buscarNodo(arbol, nombre);
